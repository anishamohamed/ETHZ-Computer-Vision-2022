...
def grid_points(img, nPointsX, nPointsY, border):
    ...
    x_points = np.linspace(border, w-1-border, nPointsX)
    y_points = np.linspace(border, h-1-border, nPointsY)

    x_grid, y_grid = np.meshgrid(x_points, y_points) # x_grid: (nPointsX, nPointsY), y_grid: (nPointsX, nPointsY)
    vPoints = np.transpose(np.reshape(np.stack((x_grid, y_grid)), (2, -1)))
    # np.stack: (nPointsX, nPointsY, 2)
    # np.reshape: (2, nPointsX*nPointsY)
    # np.transpose (nPointsX*nPointsY, 2)
...
def descriptors_hog(img, vPoints, cellWidth, cellHeight):
    ...
    full_angles = np.arctan2(grad_y, grad_x)
    # angles_max, angles_min = np.max(full_angles), np.min(full_angles)

    angles_max, angles_min = np.pi, -np.pi # +(5e-04) # adding this constant yields a better result
    descriptors = []  # list of descriptors for the current image, each entry is one 128-d vector for a grid point
    for i in range(len(vPoints)): # for each point in the grid
        center_x = round(vPoints[i, 0])
        center_y = round(vPoints[i, 1])

        desc = [] # should contain 16 (=4*4) arrays of 8 (=len(hist)) elements each
        ...
        for cell_y in range(-2, 2):
            for cell_x in range(-2, 2):
                ...
                # todo
                # compute the angles
                angles = full_angles[start_x:end_x, start_y:end_y]
                # compute the histogram
                hist, _ = np.histogram(angles, bins=nBins, range=(angles_min, angles_max))  
                # specify the range so that the histogram is computed in the same way for each grid-point...
                desc.append(hist)
...
def create_codebook(nameDirPos, nameDirNeg, k, numiter):
    ...
    # len(vFeatures) = nImgs * nPointsX * nointsY
    # Extract features for all image
    for i in tqdm(range(nImgs)): #  for each image
        ...
        vPoints = grid_points(img, nPointsX, nPointsY, border) # vPoints: (100, 2)
...
def bow_histogram(vFeatures, vCenters):
    ...
    # measure pairwise dissimilarities (here as Euclidian distance)
    # and assign each feature descriptor to the closest centroid
    diff = (vFeatures[:, np.newaxis, :] - vCenters) # array broadcasting
        # vFeatures: (M, 1, D)
        # vCenters:     (N, D)
        # diff:      (M, N, D)
    dist = np.linalg.norm(diff, axis=-1) # dist: (M, N)
        # (i, j)-th element equals to Euclidian distance of i-th feature descriptor from 
        # j-th centroid
    min_dist_idxs = np.argmin(dist, axis=-1)
    histo = np.array([(min_dist_idxs == i).sum() for i in range(vCenters.shape[0])])
...
def create_bow_histograms(nameDir, vCenters):
    ...
    for i in tqdm(range(nImgs)):
        ...
        # todo
        vPoints = grid_points(img, nPointsX, nPointsY, border) # compute grid points
        vFeatures = descriptors_hog(img, vPoints, cellWidth, cellHeight) # extract descriptors
