...
class Vgg(nn.Module):
    ...
    def forward(self, x):
        ...
        score = None
        # todo
        x = self.conv_block1(x) # x: (bs, 64, 16, 16)
        # input(f"block 1: {x.size()}")
        x = self.conv_block2(x) # x: (bs, 128, 8, 8)
        # input(f"block 2: {x.size()}")
        x = self.conv_block3(x) # x: (bs, 256, 4, 4)
        # input(f"block 3: {x.size()}")
        x = self.conv_block4(x) # x: (bs, 512, 2, 2)
        # input(f"block 4: {x.size()}")
        x = self.conv_block5(x) # x: (bs, 512, 1, 1)
        # input(f"block 5: {x.size()}")

        score = self.classifier(torch.squeeze(x))
        # input(f"classifier: {x.size()}")
